...
python3.5 -m prodmodel integration-tests/example:csv_data
...
python3.5 -m prodmodel clean integration-tests/example:csv_data --cutoff_date=2019-01-01T00:00:00
CACHE_FILES=$(ls integration-tests/example/.target/output/csv_data | wc -l)
if [ "$CACHE_FILES" -eq 0 ]
  then
    echo "No output files at integration-tests/example/.target/output/csv_data."
...
python3.5 -m prodmodel clean integration-tests/example:csv_data
CACHE_FILES=$(ls integration-tests/example/.target/output/csv_data | wc -l)
if [ "$CACHE_FILES" -gt 0 ]
  then
