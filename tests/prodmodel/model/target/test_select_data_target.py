...
class TestSelectDataTarget(TargetTestUtil):
  ...
  def test_select_data_target__keep(self):
    ...
    target = undertest.SelectDataTarget(csv_target, ['a'], keep=True, output_format='pickle')
  ...
  def test_select_data_target__discard(self):
    csv_target = CSVDataTarget(source=self.csv_data_file, dtypes={'a': int, 'b': str}, output_format='pickle')
    target = undertest.SelectDataTarget(csv_target, ['a'], keep=False, output_format='pickle')
    items = [item for item in target]
    self.assertEqual(2, len(items))
