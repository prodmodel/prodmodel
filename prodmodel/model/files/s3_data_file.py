...
class S3DataFile(InputFile):
  ...
  def __init__(self, s3_path: str):
    ...
    self.bucket = s3_path[5:s3_path.index('/', 5)]
    self.key = s3_path[s3_path.index('/', 5)  + 1:]
    self.s3 = None
  ...
  def init_impl(self, args) -> Path:
    ...
    if local_last_modified != s3_last_modified:
      ...
      logging.debug(f'Downloading s3://{self.bucket}/{self.key} to {self.file_name}.')
      with open(self.file_name, 'wb') as f:
        f.write(response['Body'].read())
      with open(metadata_file, 'w') as f:
        json.dump({'last_modified': s3_last_modified}, f)
    else:
      logging.debug(f'Using cached version of s3://{self.bucket}/{self.key}: {self.file_name}.')
